...
export function encrypt_with_pi(text, key) {
    ...
    for (let i = 0; i < text.length; i++) {
        ...
        let u_new, marker;
        if (i % 2 === 0 && p % 2 === 0) {
            u_new = u + i + p +Number(String(key).substring(0,4)); // Add OFFSET to ensure u_new is positive
            marker = '1';
        } else if (i % 2 === 0 && p % 2 === 1) {
            u_new = u + i - p +Number(String(key).substring(0,4)); // Add OFFSET
            marker = '2';
        } else if (i % 2 === 1 && p % 2 === 1) {
            u_new = u - i - p +Number(String(key).substring(0,4)); // Add OFFSET
            marker = '3';
...
export function decrypt_with_pi(text, key) {
    ...
    while (i < text.length) {
        ...
        const u_new = char.charCodeAt(0) - Number(String(key).substring(0,4)); // Subtract OFFSET to restore the original value
